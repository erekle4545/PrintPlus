...
const CreatePage = () => {
    ...
    //Get Config Options
    const getPageConfig = () => {
        http.get('options/page').then((response) => {
            if (response.status === 200) {
                setPageTemplate(response.data.data.templates)
        ...
        http.get(`page/${params.id}`, {
            ...
            if (response.status === 200) {
                if (params.id) {
                    ...
                    setValue('keywords', response.data.data.info.meta_keywords);
                    setValue('tasks', response.data.data.tasks);
                    setValue('description', response.data.data.info.description);
                    setText(response.data.data.info.text)
                    setSelectedPageTemplate(response.data.data.template_id)
    ...
    const create = (data) => {
        if (params.id) {
            // update Page
            http.post('page/update/' + params.id,
                {
                    ...data,
                    language_id: state.form_active_lang.activeLangId ? state.form_active_lang.activeLangId : null,
                    template_id: selectedPageTemplate,
            ...
            http.post('page',
                ...
                if (response.status === 200) {
                    ...
                    navigate(`/page/edit/` + response.data.id)
                }
            }).catch(err => {
                console.log(err.response)
                errorAlerts(err.response.status, err.response.statusText, err.response.data.errors)
                    ...
                    <form className="row " onSubmit={handleSubmit(create)}>
                                    ...
                                    <div className="row justify-content-between">
                                        <div className="col-xl-4 col-sm-6 ">
                                            <h5 className="card-title mb-0">
                                                ...
                                                {params.id ? translate('editPage', state.lang.code) : translate('addPage', state.lang.code)}
                                            </h5>
                                        </div>
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className='row'>
                                            ...
                                            <div className="col-xl-4 col-md-4  col-sm-12">
                                                <div className="mb-3">
                                                    <label htmlFor="page_template " className='pb-2 font_form_title font-weight-bold'>აირჩიეთ გვერდის შაბლონი</label>
                                                ...
                                                <div className="mb-3">
                                                    <label htmlFor="menuSwitch" className='pb-2 font_form_title font-weight-bold'>გამოჩნდეს მთავარ გვერდზე</label>
                                                    <div className='input-style-border-switch'>
                                                        <Switch id="menuSwitch" checked={showHomePage} onChange={(e) => setShowHomePage(e.target.checked)} />
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div className="col-md-3 col-xl-3 bg-w my-left-side-toolbar">
                            ...
                            <div className="card" style={{ background: "#f9f9f9" }}>
                                <UseAddedBar name={'page'} deleteUrl={'page/delete'} getUrl={'page/side'} />
                            </div>
                        </div>
                    </form>
                </div>
