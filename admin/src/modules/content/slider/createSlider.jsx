...
const CreateSlider = () => {
    ...
    const create = (data) => {
        if (params.id) {
            // update Page
            http.post('slider/update/' + params.id,
                {
                    ...data,
            ...
            http.post('slider',
                ...
                if (response.status === 200) {
                    ...
                    navigate(`/slider/edit/` + response.data.id)
                }
            }).catch(err => {
                console.log(err.response)
                errorAlerts(err.response.status, err.response.statusText, err.response.data.errors)
                    ...
                    <form className="row " onSubmit={handleSubmit(create)}>
                        ...
                        <div className="col-md-3 col-xl-3 bg-w my-left-side-toolbar">
                            ...
                            <div className="card" style={{ background: "#f9f9f9" }}>
                                <UseAddedBar name={'slider'} deleteUrl={'slider/delete'} getUrl={'slider/side'} />
                            </div>
                        </div>
                    </form>
                </div>
