...
const MediaList = () => {
    ...
    const checkedDelete = (e) => {
        if (e.target.checked === true) {
            ...
            dispatch({ type: 'CART', payload: [...check, e.target.value] })
            toast.success('ფაილი მონიშნულია')
        } else {
            dispatch({ type: 'CART', payload: check.filter(item => item !== e.target.value) })
            setCheck(check.filter(item => item !== e.target.value))
            toast.info('ფაილი ამოღებულია მონიშნულების სიიდან')
        }
    }
            ...
            return (
                <li key={index} className='col-xl-2 col-sm-6 text-center' title={item.name}>
                    <label htmlFor={item.id}>
                        <img src={
                            item.extension === 'jpg' || item.extension === 'png' || item.extension === 'jpeg' ? FileEndpoint + '/' + item.output_path :
                                item.extension === 'mp4' || item.extension === 'webm' ? '/img/media/video_prev.png' :
                                    item.extension === 'pdf' || item.extension === 'dox' ? '/img/media/doc.jpg' : FileEndpoint+ '/' + item.output_path
                        } alt={item.name} />
                    </label>
                    <div className='d-flex media-file-tools'>
                        <div className='d-flex'>
                            <div title='delete' onClick={() => deleteAlert(item.id, deleteRow)}><i className="align-middle me-2 fas fa-fw fa-trash"></i></div>
                            {/*<CopyToClipboard text={FileEndpoint + '/' + item.output_path}>*/}
                                <div title='Copy Link'>
                                    <i className="align-middle me-2 fas fa-fw fa-link"></i>
                                </div>
                            {/*</CopyToClipboard>*/}
                            <div title='Eye'><i className="align-middle me-2 fas fa-fw fa-eye"></i></div>
