...
// Get Slider thumbImage
export const ThumbImage = (pathData,type) => {
    const filtered =   pathData&&pathData.filter(item=>item.cover_type === type)?.[0]
    return '/'+filtered.output_path;
    // return  pathData&&pathData.map((item)=>{
    //     if(pathData.length > 0){
    //         if(type && item.cover_type === type){
    //             return '/'+item.path;
    //         }else{
    //             return '/images/no_image.jfif';
    //         }
    //     }
    // })
}
