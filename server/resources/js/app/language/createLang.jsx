...
const CreateLang = () => {
    ...
    const handleChangeSwitchLangStatus = (event) => {
        ...
        http.put(`lang/update_status/${event.target.value}`,{
            ...
            if(response.status === 200){
                ...
                alert.success(`ენის აქტივაციის სტატუსი შეიცვლა`);
    ...
    const handleChangeSwitchLangDefault = (event) => {
        http.put(`lang/update_default/${event.target.value}`,{
            ...
            if(response.status === 200){
                ...
                alert.success(`მთავარი ენის ცვლილება განხორციელდა`);
    ...
    const addLanguage = () => {
       const ifIsSetCountry = langData.filter(item => item.code === chooseLang.code)
        if(ifIsSetCountry.length === 0){
            http.post("languages",{
                ...
                if(response.status === 200){
                    ...
                    alert.success(`${chooseLang.label} ენა წარმატებით დაემატა`)
                }
            }).catch(err=>{
                console.log(err.response)
                Swal.fire({
        ...
        alert.error(`${chooseLang.label} უკვე დამატებულია!`)
        }
    }
    ...
    const drop = e => {
        ...
        // e.target.appendChild(card);
          //  console.log( e.target.__reactFiber$uypo3zgitzm.tag)
    }
