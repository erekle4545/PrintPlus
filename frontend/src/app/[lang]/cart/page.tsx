
"use client";

import Image from "next/image";
import { X } from "lucide-react";
import styles from "./Cart.module.css";
import { useCart } from "@/shared/hooks/useCart";
import Cover from "@/shared/components/theme/header/cover/cover";
import CartIsEmptyIcon from "@/shared/assets/icons/cart/bg-inside.svg";
import Button from "@/shared/components/ui/button/Button";
import { useRouter } from "next/navigation";
import RemoveIcon from "@/shared/assets/icons/delete/remove.svg";
import WhiteShoppingCart from "@/shared/assets/icons/cart/shopping-cart-white.svg";
import { useState } from "react";
import CustomLoader from "@/shared/components/ui/loader/customLoader";

export default function Cart() {
    const router = useRouter();
    const { items, updateQuantity, removeItem, clearCart, total, loading } = useCart();
    const [removingId, setRemovingId] = useState<number | null>(null);

    const handleDecrease = async (id: number, qty: number) => {
        if (qty <= 1) return;
        await updateQuantity(id, qty - 1);
    };

    const handleIncrease = async (id: number, qty: number) => {
        await updateQuantity(id, qty + 1);
    };

    const handleRemove = async (id: number) => {
        setRemovingId(id);
        await removeItem(id);
        setRemovingId(null);
    };

    const handleBuyAll = () => {
        // გადავიდეთ checkout გვერდზე
        router.push('/checkout');
    };

    const handleClearCart = async () => {
        if (window.confirm('დარწმუნებული ხართ რომ გსურთ კალათის გასუფთავება?')) {
            await clearCart();
        }
    };

    if (items.length === 0 && !loading) {
        return (
            <div className={`container text-center ${styles.cartWrapper}`}>
                <h2 className={'fw-bolder p-2 text-center title_font'}>კალათა</h2>
                <div className='text-muted'>
                    <h3 className='title_font'>კალათა ცარიელია</h3>
                    <CartIsEmptyIcon/>
                    <div>
                        <Button
                            className={'m-5'}
                            type={'button'}
                            onClick={() => router.push('/')}
                            variant={'my-btn-blue'}
                            disabled={loading}
                        >
                            ნახე პროდუქტები
                        </Button>
                    </div>
                </div>
            </div>
        );
    }

    if(loading && items.length === 0){
        return  <CustomLoader/>;
    }

    return (
        <>
            <Cover/>
            <div className={`container ${styles.cartWrapper}`}>
                <h2 className={'fw-bolder p-2 text-center title_font'}>კალათა</h2>


                    <>
                        <div className={styles.cartList}>
                            {items.map((item) => {
                                const hasDiscount = !!item.discount && item.discount > 0;
                                const discountedPrice = hasDiscount
                                    ? item.price * (1 - (item.discount ?? 0) / 100)
                                    : item.price;

                                const isRemoving = removingId === item.id;

                                return (
                                    <div
                                        key={item.id}
                                        className={`justify-content-between align-items-center text-center ${styles.cartItem} ${isRemoving ? 'opacity-50' : ''}`}
                                    >
                                        {/* left side - image */}
                                        <div className={styles.itemImageWrap}>
                                            {item.image ? (
                                                <Image
                                                    src={item.image}
                                                    alt={item.name}
                                                    width={120}
                                                    height={120}
                                                    className={styles.itemImage}
                                                />
                                            ) : (
                                                <Image
                                                    src={'/assets/img/products/pro_1.png'}
                                                    alt={item.name}
                                                    width={120}
                                                    height={120}
                                                    className={styles.itemImage}
                                                />
                                            )}
                                        </div>

                                        {/* item info */}
                                        <div className={styles.itemInfo}>
                                            <div className={styles.itemTitle}>{item.name}</div>
                                            {item.color && (
                                                <div className={styles.itemSubtitle}>
                                                    ფერი: {item.color}
                                                </div>
                                            )}
                                            {item.size && (
                                                <div className={styles.itemSubtitle}>
                                                    ზომა: {item.size}
                                                </div>
                                            )}
                                            {item.materials && (
                                                <div className={styles.itemSubtitle}>
                                                    მასალა: {item.materials}
                                                </div>
                                            )}
                                            {item.print_type && (
                                                <div className={styles.itemSubtitle}>
                                                    ბეჭდვა: {item.print_type}
                                                </div>
                                            )}
                                        </div>

                                        {/* quantity */}
                                        <div className={styles.qtyBlock}>
                                            <span className={styles.qtyLabel}>რაოდენობა</span>
                                            <div className={styles.qtyBox}>
                                                <button
                                                    type="button"
                                                    className={styles.qtyBtn}
                                                    onClick={() => handleDecrease(item.id, item.quantity)}
                                                    disabled={loading || item.quantity <= 1 || isRemoving}
                                                >
                                                    −
                                                </button>
                                                <span className={styles.qtyValue}>
                                                    {item.quantity}
                                                </span>
                                                <button
                                                    type="button"
                                                    className={styles.qtyBtn}
                                                    onClick={() => handleIncrease(item.id, item.quantity)}
                                                    disabled={loading || isRemoving}
                                                >
                                                    +
                                                </button>
                                            </div>
                                        </div>

                                        {/* price */}
                                        <div className={styles.priceBlock}>
                                            <span className={styles.priceLabel}>ფასი</span>
                                            <div className={styles.priceRow}>
                                                <span className={styles.priceCurrent}>
                                                    {discountedPrice} ₾
                                                </span>
                                                {hasDiscount && (
                                                    <>
                                                        <span className={styles.priceOld}>
                                                            {item.price} ₾
                                                        </span>
                                                        <span className={styles.discountBadge}>
                                                            -{item.discount}%
                                                        </span>
                                                    </>
                                                )}
                                            </div>
                                        </div>

                                        {/* right side delete */}
                                        <div className={styles.itemRight}>
                                            <button
                                                title={'წაშლა'}
                                                type="button"
                                                className={styles.removeBtn}
                                                onClick={() => handleRemove(item.id)}
                                                aria-label="წაშლა"
                                                disabled={isRemoving || loading}
                                            >
                                                {isRemoving ? (
                                                    <div className="spinner-border spinner-border-sm" role="status">
                                                        <span className="visually-hidden">წაშლა...</span>
                                                    </div>
                                                ) : (
                                                    <X size={18} />
                                                )}
                                            </button>
                                        </div>
                                    </div>
                                );
                            })}
                        </div>

                        {/* bottom bar */}
                        <div className={styles.bottomBar}>
                            <div className="d-flex justify-content-between align-items-center gap-2">
                                <div>
                                    <h5 className="m-1 text_font">ჯამური თანხა:</h5>
                                </div>
                                <div>
                                    <h3 className="fw-bolder m-0 text_font">
                                        {total.toFixed(2)}₾
                                    </h3>
                                </div>
                            </div>

                            <div className={styles.bottomActions}>
                                <Button
                                    type={'button'}
                                    onClick={handleClearCart}
                                    startIcon={<RemoveIcon/>}
                                    variant={'my-btn-danger'}
                                    disabled={loading}
                                >
                                    {loading ? 'დამუშავება...' : 'ყველას წაშლა'}
                                </Button>

                                <Button
                                    type={'button'}
                                    onClick={handleBuyAll}
                                    startIcon={<WhiteShoppingCart/>}
                                    variant={'my-btn-blue'}
                                    disabled={loading}
                                >
                                    შემდეგი გვერდი
                                </Button>
                            </div>
                        </div>
                    </>
            </div>
        </>
    );
}
// "use client";
//
// import Image from "next/image";
//
// import {  X } from "lucide-react";
// import styles from "./Cart.module.css";
// import {useCart} from "@/shared/hooks/useCart";
// import Cover from "@/shared/components/theme/header/cover/cover";
// import CartIsEmptyIcon from "@/shared/assets/icons/cart/bg-inside.svg";
// import Button from "@/shared/components/ui/button/Button";
// import {useRouter} from "next/navigation";
// import RemoveIcon from "@/shared/assets/icons/delete/remove.svg";
// import WhiteShoppingCart from "@/shared/assets/icons/cart/shopping-cart-white.svg";
//  export default function Cart() {
//      const router = useRouter();
//
//     const { items, updateQuantity, removeItem, clearCart, total } = useCart();
//
//     const handleDecrease = (id: number, qty: number) => {
//         if (qty <= 1) return;
//         updateQuantity(id, qty - 1);
//     };
//
//     const handleIncrease = (id: number, qty: number) => {
//         updateQuantity(id, qty + 1);
//     };
//
//     const handleBuyItem = (id: number) => {
//         // TODO: აქ შეაერთე checkout ლოგიკას
//         console.log("Buy single item", id);
//     };
//
//     const handleBuyAll = () => {
//         // TODO: აქცეც checkout all ლოგიკა
//         console.log("Buy all items", items);
//     };
//
//     if (items.length === 0) {
//         return (
//             <div className={`container text-center ${styles.cartWrapper}`}>
//                 <h2 className={'fw-bolder p-2 text-center title_font'}>კალათა</h2>
//                 <div className=' text-muted '>
//                     <h3 className='title_font'>კალათა ცარიელია </h3>
//                     <CartIsEmptyIcon/>
//                     <div>
//                         <Button className={'m-5'} type={'button'}  onClick={()=>  router.push('/') } variant={'my-btn-blue'}  > ნახე პროდუქტები </Button>
//                     </div>
//                 </div>
//             </div>
//         );
//     }
//
//     return (
//             <>
//                 <Cover/>
//                 <div className={`container ${styles.cartWrapper}`}>
//                     <h2 className={'fw-bolder p-2 text-center title_font'}>კალათა</h2>
//                     {/*<HeaderTitle title="კალათა" slug={[]} />*/}
//
//                     <div className={styles.cartList}>
//                         {items.map((item) => {
//                             const hasDiscount = !!item.discount && item.discount > 0;
//                             const discountedPrice = hasDiscount
//                                 ? item.price * (1 - (item.discount ?? 0) / 100)
//                                 : item.price;
//                             return (
//                                 <div key={item.id} className={` justify-content-between align-items-center text-center ${styles.cartItem}`}>
//                                     {/* left side */}
//                                     <div className={styles.itemImageWrap}>
//                                         <Image
//                                             src={'/assets/img/products/pro_1.png'}
//                                             alt={item.name}
//                                             width={120}
//                                             height={120}
//                                             className={styles.itemImage}
//                                         />
//                                     </div>
//
//                                     <div className={styles.itemInfo}>
//                                         <div className={styles.itemTitle}>{item.name}</div>
//                                          {/*<div className={styles.itemSubtitle}>ბალიში ნარინჯისფერი</div>*/}
//                                     </div>
//
//                                     {/* qty */}
//                                     <div className={styles.qtyBlock}>
//                                         <span className={styles.qtyLabel}>რაოდენობა</span>
//                                         <div className={styles.qtyBox}>
//                                             <button
//                                                 type="button"
//                                                 className={styles.qtyBtn}
//                                                 onClick={() =>
//                                                     handleDecrease(item.id, item.quantity)
//                                                 }
//                                             >
//                                                 −
//                                             </button>
//                                             <span className={styles.qtyValue}>
//                                                 {item.quantity}
//                                             </span>
//                                             <button
//                                                 type="button"
//                                                 className={styles.qtyBtn}
//                                                 onClick={() =>
//                                                     handleIncrease(item.id, item.quantity)
//                                                 }
//                                             >
//                                                 +
//                                             </button>
//                                         </div>
//                                     </div>
//
//                                     {/* price */}
//                                     <div className={styles.priceBlock}>
//                                         <span className={styles.priceLabel}>ფასი</span>
//                                         <div className={styles.priceRow}>
//                                             <span className={styles.priceCurrent}>
//                                                 {discountedPrice.toFixed(2)}₾
//                                             </span>
//                                             {hasDiscount && (
//                                                 <>
//                                                     <span className={styles.priceOld}>
//                                                         {item.price.toFixed(2)}₾
//                                                     </span>
//                                                     <span className={styles.discountBadge}>
//                                                         -{item.discount}%
//                                                     </span>
//                                                 </>
//                                             )}
//                                         </div>
//                                     </div>
//
//
//
//                                     {/* right side delete  */}
//                                     <div className={styles.itemRight}>
//
//                                             <button
//                                                 title={'წაშლა'}
//                                                 type="button"
//                                                 className={styles.removeBtn}
//                                                 onClick={() => removeItem(item.id)}
//                                                 aria-label="წაშლა"
//                                             >
//                                                 <X size={18} />
//                                             </button>
//
//                                     </div>
//                                 </div>
//                             );
//                         })}
//                     </div>
//
//                     {/* ქვედა ზოლი – ყველა ერთად */}
//                     <div className={styles.bottomBar}>
//                         <div className="d-flex justify-content-between align-items-center gap-2 ">
//                             <div>
//                                 <h5 className="m-1 text_font">ჯამური თანხა:</h5>
//                             </div>
//                             <div>
//                                 <h3 className="fw-bolder m-0 text_font">{total.toFixed(2)}₾</h3>
//                             </div>
//                         </div>
//
//                         <div className={styles.bottomActions}>
//
//                             <Button
//                                 type={'button'}
//                                 onClick={clearCart}
//                                 startIcon={<RemoveIcon/> }
//                                 variant={'my-btn-danger'}
//                             >ყველას წაშლა</Button>
//
//                             <Button
//                                 type={'button'}
//                                 onClick={handleBuyAll}
//
//                                 startIcon={<WhiteShoppingCart/>}
//                                 variant={'my-btn-blue'}
//                             >შემდეგი გვერდი</Button>
//                         </div>
//                     </div>
//                 </div>
//             </>
//     );
//  }
