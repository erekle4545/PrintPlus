...
export default function CalculatePage({ page, products }: CalculatePageProps) {
    ...
    useEffect(() => {
        if (products.length > 0 && activeProduct === null) {
            ...
            if (firstProduct.materials && firstProduct.materials.length > 0) {
                setActiveMaterial(firstProduct.materials[0].id);
            }

            if (firstProduct.sizes && firstProduct.sizes.length > 0) {
                setActiveSize(firstProduct.sizes[0].id);
            }
        }
    ...
    const selectedMaterial = useMemo(() =>
            selectedProduct?.materials?.find(m => m.id === activeMaterial),
        [selectedProduct, activeMaterial]
    );

    const selectedSize = useMemo(() =>
            selectedProduct?.sizes?.find(s => s.id === activeSize),
    ...
    const sizePrice = useMemo(() => {
        ...
        const price = selectedSize.pivot?.price || selectedSize.base_price;
        return parseFloat(price.toString()) * m2;
    }, [selectedSize, m2]);

    // მატერიალის ფასი
    const materialPrice = useMemo(() => {
        if (!selectedMaterial) return 0;
        const price = selectedMaterial.pivot?.price || selectedMaterial.base_price;
        return parseFloat(price.toString());
    }, [selectedMaterial]);

    // დამატებითი მომსახურებების ფასი
    const extrasTotal = useMemo(() => {
        if (!selectedProduct?.extras) return 0;
        return selectedProduct.extras
            .filter(extra => selectedExtras.includes(extra.id))
            .reduce((sum, extra) => {
                const price = extra.pivot?.price || extra.base_price;
                            ...
                            {selectedProduct?.materials && selectedProduct.materials.length > 0 && (
                                <ul className="nav nav-pills seg-pills gap-2 mb-4 flex-wrap title_font_bold">
                                    {selectedProduct.materials.map((material) => {
                                        const activeCls = activeMaterial === material.id ? 'btn-teal' : 'btn-outline-secondary';
                                        return (
                                            <li key={material.id} className="nav-item">
                                                <button
                                                    className={`btn ${activeCls} px-3 py-2`}
                                ...
                                <div className='d-flex justify-content-center'>
                                    <div className="mt-3">
                                        {selectedProduct.extras.map((extra) => {
                                            const extraPrice = extra.pivot?.price || extra.base_price;
                                            return (
